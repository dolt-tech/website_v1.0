...
export default function Section() {
  ...
  const duration = 30_000 // 30sec 
  const target = 100
  const intervalTime = 100
  const increment = (target / duration) * intervalTime
  const sentence = "D0LT delivers smarter maintenance with AI  automation and real-time monitoring"
  ...
  const interval = setInterval(() => {
    setCount((prev) => {
      if (prev + increment >= target) {
        return 0 // reset to 0 once 100 is reached
      }
...
<div className="absolute bottom-24 right-6 max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-5xl z-10">
  ...
  >
   ...
   {/* Right side line + text */}
<div className="flex flex-wrap absolute top-12 right-6 items-center z-10">
  <span className="inline-block w-64 h-1 bg-white mr-4" /> {/* thicker and longer */}
